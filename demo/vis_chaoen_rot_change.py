...
def img2video_ori(video_path, output_dir):
    ...
    names = sorted(glob.glob(os.path.join(output_dir + "pose2D_ori/", "*.png")))
    img = cv2.imread(names[0])
    size = (img.shape[1], img.shape[0])
    output_video_ori_dir = os.path.join(output_dir, "output_video_ori")
    os.makedirs(output_video_ori_dir, exist_ok=True)
    ...
    for segment_index, (start_frame, end_frame) in tqdm(enumerate(start_end_frames)):
        ...
        video_write_segment = cv2.VideoWriter(os.path.join(output_video_ori_dir, segment_name), fourcc, fps, size)
        segment_writers.append((start_frame, end_frame, video_write_segment))

    # 錄製影片
    for frame_idx, name in tqdm(enumerate(names)):
        img = cv2.imread(name)

        # 錄製一段段影片
        for start_frame, end_frame, video_write_segment in segment_writers:
            if start_frame <= frame_idx <= end_frame:
                video_write_segment.write(img)

    # 釋放資源
    for _, _, video_write_segment in segment_writers:
        video_write_segment.release()

    print("Save original video successful!")
...
def img2video(video_path, output_dir):
    ...
    # 初始化錄製整部影片
    video_write_full = cv2.VideoWriter(os.path.join(output_video_dir, video_name + "_full.mp4"), fourcc, fps, size)
    ...
    for segment_index, (start_frame, end_frame) in tqdm(enumerate(start_end_frames)):
        segment_name = f"{video_name}_{segment_index:02d}.mp4"
        video_write_segment = cv2.VideoWriter(os.path.join(output_video_dir, segment_name), fourcc, fps, size)
        segment_writers.append((start_frame, end_frame, video_write_segment))
    ...
    # 釋放資源
    video_write_full.release()
    for _, _, video_write_segment in segment_writers:
        video_write_segment.release()

    print("Save 3D pose video successful!")
...
def save_data(video_post_out, output_dir, video_name, label):
    ...
    for segment_index, row in df.iterrows():
        ...
        print(f"Save 3D pose data for segment {segment_index} successful!")
...
def get_pose3D(video_path, output_dir):
    ...
    for i in tqdm(range(video_length)):
        ...
        cv2.imwrite(output_dir_2D + str(("%05d" % i)) + "_2D.png", image)

        output_ori_dir_2D = output_dir + "pose2D_ori/"
        os.makedirs(output_ori_dir_2D, exist_ok=True)
        cv2.imwrite(output_ori_dir_2D + str(("%05d" % i)) + "_2D_ori.png", img)
    ...
    for idx, clip in enumerate(clips):
        ...
        for j, post_out in tqdm(enumerate(post_out_all)):
            ...
            str(("%05d" % (idx * 243 + j)))
            plt.savefig(
                output_dir_3D + str(("%05d" % (idx * 243 + j))) + "_3D.png", dpi=200, format="png", bbox_inches="tight"
            )
            plt.close(fig)
    ...
    for i in tqdm(range(len(image_2d_dir))):
        ...
        plt.margins(0, 0)
        plt.savefig(output_dir_pose + str(("%05d" % i)) + "_pose.png", dpi=200, bbox_inches="tight")
        plt.close(fig)

    return video_post_out
...
if __name__ == "__main__":
    ...
    video_paths = (
        [os.path.join(video_path, filename) for filename in sorted(os.listdir(video_path))]
